...
import electionguard.makeDoerreTrustee
import electionguard.makeGuardian
...
class TallyDecryptorTest {
    ...
    fun testGood() {
        ...
        val decryptedTally2 =
            tallyDecryptor.decryptTally(tally, decryptions, Stats(), ErrorMessages("TallyDecryptorTest3"))
    ...
    internal fun decrypt(
        ...
        if (messItUp) {
            val decryptionResult: SelectionDecryptionResult = trusteeDecryptions[1].shares["Contest1#@Selection1"]!!
            val partialDecryptionMessed: PartialDecryption = decryptionResult.share.copy(a = group.ONE_MOD_P)
            trusteeDecryptions[1].shares["Contest1#@Selection1"] =
                decryptionResult.copy(share = partialDecryptionMessed)
        }
        ...
        for (trustee in decryptingTrustees) {
            trusteeChallengeResponses.add(
                allDecryptions.challengeTrustee(
                    trustee,
                    tallyDecryptor.lagrangeCoordinates[trustee.id()]
                )
            )
        }
    ...
    fun AllDecryptions.challengeTrustee(
        trustee: DecryptingTrusteeIF,
        lagrangeCoordinate: LagrangeCoordinate?
