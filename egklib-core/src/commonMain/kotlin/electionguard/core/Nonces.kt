...
class Nonces(seed: ElementModQ, vararg headers: Any) {
    ...
    val internalSeed =
        if (headers.isNotEmpty()) hashFunction(seed.byteArray(), null, *headers).bytes else seed.byteArray()
...
fun Nonces.getWithHeaders(index: Int, vararg headers: String) =
    hashFunction(internalSeed, null, index, *headers).toElementModQ(internalGroup)
